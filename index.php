...
function playChannel(i) {
    ...
    // বিজ্ঞাপন লজিক (নতুন ডাটাবেস কলাম অনুযায়ী আপডেট)
    if (c.ads_status === true && c.ad_enabled === true && c.ad_url) {
        showAd(c.ad_url, c.ad_duration || 30);
    }

    // হেডলাইন টেক্সট ও সেটিংস (নতুন ডাটাবেস কলাম অনুযায়ী আপডেট)
    const h = document.getElementById('headline');
    h.innerText = c.ticker_enabled ? c.ticker_text : "";
    h.style.animationDuration = (c.ticker_speed || 40) + 's';
    h.style.direction = (c.ticker_direction === 'right') ? 'rtl' : 'ltr';

    // লাইভ বাটন টেক্সট ও সেটিংস (নতুন ডাটাবেস কলাম অনুযায়ী আপডেট)
    document.getElementById('liveBtn').innerText = c.live_enabled ? c.live_text : "LIVE";
    
    showNameAuto(c.name);

    // ভিডিও প্লেয়ারের মূল লজিক (অক্ষত)
