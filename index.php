...
async function fetchData(){try{const r=await fetch('channels_api.php?did='+myDeviceId,{cache:"no-cache"});const d=await r.json();if(d.status==="inactive"){document.body.innerHTML=`<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;background:#000;color:#fff;padding:20px;text-align:center;"><div style="background:#222;padding:15px;border-radius:10px;margin-bottom:20px;width:100%;max-width:400px;">সফটওয়্যারটি লক করা আছে!</div><h2 style="color:#00ff00;">${myDeviceId}</h2><button onclick="showActivationForm()" style="padding:15px;background:#1565d8;color:#fff;border:none;margin-top:20px;">যোগাযোগ</button></div>`;return;}channels=d.channels;renderList();if(channels.length>0){let l=parseInt(localStorage.getItem('last_channel')||0);if(l>=channels.length)l=0;playChannel(l);}}catch(e){}}
