<?php
/* File: admin/free/edit_channels.php - FINAL COMPLETE MASTER CODE */
session_start();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    header("Location: edit_channels.php?id=$id"); exit();
}
...
<head>
    ...
    <title>Edit Channel Master</title>
...
<div style="max-width:600px; margin:auto;">
    <a href="channels.php" class="text-warning text-decoration-none">← Back to List</a>
    <h3 class="text-center text-warning my-3">CHANNEL EDIT: <?php echo htmlspecialchars($c['channel_name']); ?></h3>
    ...
    <form method="POST" class="box">
        ...
        <select name="channel_order" class="form-select"><?php for($i=1;$i<=20;$i++):?><option value="<?php echo $i;?>" <?php echo ($c['channel_order']==$i?'selected':'');?>>Channel Order <?php echo $i;?></option><?php endfor;?></select>
        <button type="submit" class="btn btn-gold">SAVE BASIC SETTINGS</button>
    </form>

    <form method="POST" class="box">
        <input type="hidden" name="action" value="live">
        <label>2. Live Status Settings</label>
        ...
        <button type="submit" class="btn btn-gold">SAVE LIVE SETTINGS</button>
    </form>

    <form method="POST" class="box">
        <input type="hidden" name="action" value="ticker">
        <label>3. Headline (Ticker) Settings</label>
        <input type="text" name="ticker_text" class="form-control" value="<?php echo htmlspecialchars($c['ticker_text']); ?>">
        <select name="ticker_enabled" class="form-select"><option value="1" <?php echo ($c['ticker_enabled']==1?'selected':'');?>>Show</option><option value="0" <?php echo ($c['ticker_enabled']==0?'selected':'');?>>Hide</option></select>
        ...
        <button type="submit" class="btn btn-gold">SAVE TICKER SETTINGS</button>
    </form>

    <form method="POST" class="box">
        <input type="hidden" name="action" value="ads">
        <label>4. Advertisement Settings</label>
        <input type="text" name="ad_url" class="form-control" value="<?php echo htmlspecialchars($c['ad_url']); ?>">
        <select name="ads_status" class="form-select"><option value="1" <?php echo ($c['ads_status']==1?'selected':'');?>>Active</option><option value="0" <?php echo ($c['ads_status']==0?'selected':'');?>>Inactive</option></select>
        <select name="ad_type" class="form-select"><option value="short" <?php echo ($c['ad_type']=='short'?'selected':'');?>>Short Video</option><option value="long" <?php echo ($c['ad_type']=='long'?'selected':'');?>>Long Video</option></select>
        <select name="ad_size" class="form-select"><?php for($i=20;$i<=100;$i+=10):?><option value="<?php echo $i;?>" <?php echo ($c['ad_size']==$i?'selected':'');?>>Size <?php echo $i;?>%</option><?php endfor;?></select>
        <select name="ad_duration" class="form-select"><?php for($i=5;$i<=60;$i+=5):?><option value="<?php echo $i;?>" <?php echo ($c['ad_duration']==$i?'selected':'');?>>Duration <?php echo $i;?> Seconds</option><?php endfor;?></select>
        <button type="submit" class="btn btn-gold">SAVE AD SETTINGS</button>
    </form>
</div>
</body>
</html>
