<?php
/* File: admin/free/edit_channels.php - FINAL DYNAMIC MASTER - FIXED */
session_start();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == 'basic') {
        ...
        // ফিক্সড: এখানে ৬টি কলামের জন্য ৬টি "?" এবং ৬টি ডাটা টাইপ ব্যবহার করা হয়েছে
...
<div style="max-width:600px; margin:auto;">
    ...
    <form method="POST" class="box">
        ...
        <select name="ad_enabled" class="form-select"><option value="1" <?php echo ($c['ad_enabled']==1?'selected':''); ?>>Ads ON</option><option value="0" <?php echo ($c['ad_enabled']==0?'selected':''); ?>>Ads OFF</option></select>
        <label>Ads Status</label>
        <input type="number" name="ads_status" class="form-control" value="<?php echo $c['ads_status']; ?>">
        <label>Ad Type & Size</label>
        <input type="text" name="ad_type" class="form-control" value="<?php echo htmlspecialchars($c['ad_type']); ?>">
        <input type="number" name="ad_size" class="form-control" value="<?php echo $c['ad_size']; ?>">
