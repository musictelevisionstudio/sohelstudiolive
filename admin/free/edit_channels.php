<?php
/* File: admin/free/edit_channels.php - FULL SCREEN & ERROR-FREE */
session_start();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // ডাটাবেসের টেবিল অনুযায়ী সব কলামের আপডেট কুয়েরি
...
<head>
    ...
    <style>
        body { background: #000; color: #fff; margin: 0; padding: 10px; min-height: 100vh; }
        .edit-container { width: 100%; max-width: 100%; border: 2px solid gold; padding: 15px; border-radius: 15px; background: #111; }
        .form-control, .form-select { background: #222 !important; border: 1px solid #555 !important; color: #fff !important; margin-bottom: 10px; }
        label { color: gold; font-weight: bold; font-size: 0.85rem; }
        .btn-gold { background: gold; color: #000; font-weight: bold; width: 100%; font-size: 18px; }
    </style>
</head>
<body>
<div class="edit-container">
    <a href="channels.php" class="text-warning text-decoration-none">← Back</a>
    <h4 class="text-warning text-center mb-3">EDIT CHANNEL</h4>
    <?php echo $msg; ?>
    <form method="POST">
        ...
        <label>Order</label><input type="number" name="channel_order" class="form-control" value="<?php echo $channel['channel_order'] ?? 0; ?>">
        <label>URL</label><input type="text" name="channel_url" class="form-control" value="<?php echo htmlspecialchars($channel['channel_url'] ?? ''); ?>">
        
        <div class="row">
            <div class="col-4"><label>Live Text</label><input type="text" name="live_text" class="form-control" value="<?php echo htmlspecialchars($channel['live_text'] ?? ''); ?>"></div>
            <div class="col-4"><label>Animation</label><select name="live_animation" class="form-select"><option value="pulse" <?php if(($channel['live_animation']??'')=='pulse') echo 'selected';?>>Pulse</option><option value="blink" <?php if(($channel['live_animation']??'')=='blink') echo 'selected';?>>Blink</option></select></div>
            <div class="col-4"><label>Live Status</label><select name="live_enabled" class="form-select"><option value="1" <?php if(($channel['live_enabled']??0)==1) echo 'selected';?>>Show</option><option value="0" <?php if(($channel['live_enabled']??0)==0) echo 'selected';?>>Hide</option></select></div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-6"><label>Ticker Status</label><select name="ticker_enabled" class="form-select"><option value="1" <?php if(($channel['ticker_enabled']??0)==1) echo 'selected';?>>Show</option><option value="0" <?php if(($channel['ticker_enabled']??0)==0) echo 'selected';?>>Hide</option></select></div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-4"><label>Ad Type</label><select name="ad_type" class="form-select"><option value="short" <?php if(($channel['ad_type']??'')=='short') echo 'selected';?>>Short</option><option value="full" <?php if(($channel['ad_type']??'')=='full') echo 'selected';?>>Full</option></select></div>
