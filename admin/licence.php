<?php
/* File: admin/licence.php - FINAL VERSION WITH MESSAGES */
session_start();
...
if(isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if($_GET['action'] == 'toggle') {
        ...
        if($stmt->execute()) $_SESSION['msg'] = "Status Updated Successfully!";
    } 
    elseif($_GET['action'] == 'delete') {
        $stmt = $conn->prepare("DELETE FROM devices WHERE id = ?");
        $stmt->bind_param("i", $id);
        if($stmt->execute()) $_SESSION['msg'] = "Device Deleted Successfully!";
    } 
    elseif($_GET['action'] == 'update' && $_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if($stmt->execute()) $_SESSION['msg'] = "Profile Updated Successfully!";
    }
...
$query = $search ? "SELECT * FROM devices WHERE device_id LIKE ? OR name LIKE ? OR mobile LIKE ? ORDER BY id DESC" : "SELECT * FROM devices ORDER BY id DESC";
$stmt = $conn->prepare($query);
if ($search) { $stmt->bind_param("sss", $search, $search, $search); }
$stmt->execute();
...
<head>
    ...
    <style>
        ...
        .modal-content input { width: 100%; margin-bottom: 10px; padding: 10px; background: #222; border: 1px solid #444; color: #fff; border-radius: 5px; }
        .btn-sm-custom { font-size: 12px; padding: 5px 10px; }
        .date-text { font-size: 10px; color: #ffc107; }
        .id-text { font-size: 11px; color: #aaa; }
    </style>
</head>
<body>
<div class="container py-3">
    <a href="dashboard.php" class="btn btn-secondary btn-sm mb-3">← Back</a>
    <h2 class="text-center text-warning mb-4">DEVICE CONTROL PANEL</h2>
    ...
    <form method="GET" class="d-flex gap-2 mb-3">
        <input type="text" name="search" class="form-control bg-dark text-white border-secondary" placeholder="Search by ID, Name or Mobile...">
            ...
            <tbody>
                ...
                <tr>
                    <td class="text-start ps-3">
                        ...
                        <div class="id-text">ID: <?php echo htmlspecialchars($row['device_id']); ?></div>
                        <div class="id-text">Mob: <?php echo htmlspecialchars($row['mobile'] ?: 'N/A'); ?></div>
...
<div id="profileModal" class="modal" style="display:none; background:rgba(0,0,0,0.85);">
    <div class="modal-dialog modal-dialog-centered"><div class="modal-content bg-dark text-white p-4">
        <h3 class="mb-3">Edit Device Profile</h3>
        <form method="POST" id="editForm">
            <label>Name</label><input type="text" name="name" id="name">
            <label>Father's Name</label><input type="text" name="fname" id="fname">
            <label>Mother's Name</label><input type="text" name="mname" id="mname">
            <label>Address</label><input type="text" name="addr" id="addr">
